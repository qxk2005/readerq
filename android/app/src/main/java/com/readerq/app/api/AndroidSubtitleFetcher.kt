...
3. **章节标题时间戳对齐（极度重要）**：
   - 必须且仅在每一个 Markdown 章节标题（如 ## 或 ###）的结尾，标注对应视频的时间戳标记 `[MM:SS]`，例如 `## 一、背景介绍 [01:25]`；
...
3. **原文语义引用锚点（极度重要）**：
   ...
   - `#quote-` 后面的文字必须直接取自原文中能代表该段落/要点特征的连续文本片段（10-25个字），不要在文字中包含中英文引号，也不要包含英文圆括号 `(` 或 `)`，以便阅读器通过文本模糊查找高亮定位到原文对应段落；
